...
! local: 4-pt cubic
  ...
  real, parameter:: d1 =  0.5
  real, parameter:: d2 = -1./12.
...
#ifdef HI_CORNER
    if ( sw_corner ) then
        qout(1,1) = d1*(qin(1,1)+qin(1,0)+qin(0,1)) + d2*( qin(1,-1)+qin(2,0) +  &
                                          qin(2,1)+qin(1,2) + qin(-1,1)+qin(0,2) )
    endif
    if ( se_corner ) then
        qout(npx,1) = d1*(qin(npx-1,1)+qin(npx-1,0)+qin(npx,1)) + d2*( qin(npx-1,-1)+qin(npx-2,0) + & 
                          qin(npx+1,1)+qin(npx,2) + qin(npx-2,1)+qin(npx-1,2) )
    endif
    if ( ne_corner ) then
        qout(npx,npy) = d1*(qin(npx-1,npy-1)+qin(npx,npy-1)+qin(npx-1,npy)) + d2*( qin(npx-1,npy-2)+ &
                       qin(npx-2,npy-1) + qin(npx,npy-2)+qin(npx+1,npy-1)+qin(npx-2,npy)+qin(npx-1,npy+1))
    endif
    if ( nw_corner ) then
        qout(1,npy) = d1*(qin(1,npy-1)+qin(0,npy-1)+qin(1,npy)) + d2*( qin(0,npy-2)+qin(-1,npy-1)+ &
                      qin(1,npy-2)+qin(2,npy-1) + qin(2,npy)+qin(1,npy+1) )
    endif
#else
