...
def fv3_level_make_plots(ab_dict, zss, do_ylog, zcoord_type, zlimit_type=None, save=False):
    ...
    for count, config in enumerate(ab_dict):
        ...
        for zs_count, zs in enumerate(zss):
            ...
            zlimits = {'p':{'PBL': (100000, 85000)},'z':{'PBL': (0,4000)},'k':{'PBL': (30,50)}}
            ...
            ax[get_subplot_indices(plt_count, nrows, ncols)].plot(delp,zcoord_m, dashes=dash_pattern, markersize=MARKERSIZE, label=label, color=line_color, marker='o')
            ...
            plt_count += 1
            ax[get_subplot_indices(plt_count, nrows, ncols)].plot(delp[:-1]/delp[1:],zcoord_e[1:-1], dashes=dash_pattern,markersize=MARKERSIZE, color=line_color, marker='o')
            ax[get_subplot_indices(plt_count, nrows, ncols)].set_title('ratio delp')

            plt_count += 1
            ax[get_subplot_indices(plt_count, nrows, ncols)].plot(dlogp,zcoord_m, dashes=dash_pattern,markersize=MARKERSIZE, color=line_color, marker='o')
            ax[get_subplot_indices(plt_count, nrows, ncols)].set_title('dlogp [Pa]')
            
            plt_count += 1
            ax[get_subplot_indices(plt_count, nrows, ncols)].plot(dlogp[:-1]/dlogp[1:],zcoord_e[1:-1], dashes=dash_pattern,markersize=MARKERSIZE, color=line_color, marker='o')
            ax[get_subplot_indices(plt_count, nrows, ncols)].set_title('ratio dlogp')

            plt_count += 1
            ax[get_subplot_indices(plt_count, nrows, ncols)].plot(dz,zcoord_m, dashes=dash_pattern,markersize=MARKERSIZE, color=line_color, marker='o')
            ax[get_subplot_indices(plt_count, nrows, ncols)].set_title('dz[m]')
            if zlimit_type == "PBL":
                ax[get_subplot_indices(plt_count, nrows, ncols)].set_xlim(0, 1000)

            plt_count += 1
            ax[get_subplot_indices(plt_count, nrows, ncols)].plot(dz[:-1]/dz[1:],zcoord_e[1:-1], dashes=dash_pattern,markersize=MARKERSIZE, color=line_color, marker='o')
            ax[get_subplot_indices(plt_count, nrows, ncols)].set_title('ratio dz')

            plt_count += 1
            ax[get_subplot_indices(plt_count, nrows, ncols)].plot(np.linspace(npz,1,npz),zcoord_m, dashes=dash_pattern,markersize=MARKERSIZE, color=line_color, marker='o')
            ax[get_subplot_indices(plt_count, nrows, ncols)].set_title('# layers')
            if zcoord_type != 'k':
                dxax = np.diff(ax[get_subplot_indices(plt_count, nrows, ncols)].get_xlim())[0]
                ax[get_subplot_indices(plt_count, nrows, ncols)].text(1+dxax*(0.05+count*0.26),zcoord_m[-1],'dz/2 = %d m' % (zm[-1]-ze[-1]),
                    horizontalalignment='left',verticalalignment='center', color=line_color)
            if zlimit_type == "PBL":
    ...
    fig.legend(*ax[0,0].get_legend_handles_labels(), loc='upper center', bbox_to_anchor=(0.5, 0), ncol=len(ab_dict))
    if save:
